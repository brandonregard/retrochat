...
Transfer `dist/retrochat-0.0.2-windows98-test-kit.zip` through a read-only UTM
...
Attach `dist/retrochat-0.0.2-windows98-test.iso` to the Windows 98 VM's CD/DVD
...
Build the complete ANSI installer and a CD image containing it:

```sh
host$ cd /Users/brandon/retrochat
host$ make windows98-installer-iso \
  TCL805_INSTALLER="$HOME/Downloads/tcl805.exe"
```

Attach `dist/retrochat-0.0.2-windows.iso` to the VM. In Windows 98,
