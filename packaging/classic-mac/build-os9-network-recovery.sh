...
DeRez -only DATA "$work/SimpleTk68K" > "$work/data68k.r"
iconv -f UTF-8 -t MACINTOSH packaging/classic-mac/bgerror.tcl |
    LC_ALL=C tr '\n' '\r' > "$work/bgerror.tcl"
...
Rez -a -ov -o "$recovery" "$work/data68k.r"
Rez -a -ov \
    -d "BGERROR_SCRIPT_PATH=\"$work/bgerror.tcl\"" \
    -o "$recovery" packaging/classic-mac/bgerror-resource.r
